<template>
    <div class="public-schedule-wrapper">
        <header class="schedule-header">
            <h2>Plan Zajęć (Studenci)</h2>
            <div class="controls">
                <select v-model="selectedCohort" @change="fetchSchedule">
        ...
        <div class="table-responsive">
            <table class="schedule-table">
                ...
                <tbody>
                    ...
                    <tr v-else-if="schedule.length === 0"><td colspan="2" class="text-center">Brak zajęć w tym tygodniu.</td></tr>
                    <tr v-for="entry in schedule" :key="entry.id" v-else>
                        ...
                        <td>
                            <div class="entry-card">
                                ...
                                <span class="cohort" v-if="!selectedCohort">Rocznik: {{ entry.cohort.name }}</span>
                                <span class="lecturer">Wykładowca: {{ entry.lecturer.name }}</span>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
const fetchSchedule = async () => {
    ...
    try {
        ...
        // Sortujemy rosnąco po dacie i bloku (zakładając strukturę)
...
.controls select { padding: 0.5rem; border-radius: 4px; font-size: 1rem; }
...
.schedule-table th { background-color: #333; color: white; }
...
.cohort, .lecturer { font-size: 0.9rem; color: #555; }
